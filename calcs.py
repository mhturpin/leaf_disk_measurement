...
def sum_real(arr):
  return sum(v for v in arr if real_number(v))

def average(arr):
  real = [v for v in arr if real_number(v)]
  return 0.0 if not real else sum(real) / len(real)
...
def r_squared(data, coefficients):
  ...
  return 1 - (regression_sq_err / total_sq_err) if total_sq_err != 0 else 0.0
...
class PixelBlob:
  ...
  def touches(self, row, col):
    return (self.top - 1) <= row <= (self.bottom + 1) and \
         (self.left - 1) <= col <= (self.right + 1)
  ...
  @property
  def radius(self):
    return (self.height + self.width) / 4.0

  def center_coordinates(self):
    return {'center_row': (self.bottom + self.top) / 2.0,
        'center_col': (self.right + self.left) / 2.0}

  def leaf_disk(self):
    is_square            = within_tolerance(self.height, self.width, 0.1)
    expected_pixels      = math.pi * self.radius**2
    correct_pixel_count  = within_tolerance(expected_pixels, len(self.coordinates), 0.1)
    return is_square and correct_pixel_count and self.radius > 50
...
class LeafDiskImage:
  ...
  def process_image(self):
    ...
    for row in range(height):
      for col in range(width):
        ...
        if len(indices) == 0:
          ...
          blob = self.dark_blobs[indices[0]]
          blob.merge(self.dark_blobs[indices[1]])
    ...
    self.leaf_disk_blobs = [b for b in self.dark_blobs if b.leaf_disk()]
    ...
    for row in range(height):
      for col in range(width):
        ...
        if pixel.get('is_dark'):
          v = max(0, min(255, ((r - g + 50) * 255.0 / 100)))
          r, g, b = v, v, v
  ...
  def label_row_marker_blocks(self, blob, number):
    block_size = round((blob.bottom - blob.top) / 10.0)
    if number % 2 == 1:
  ...
  def calculate_disk_score(self, blob):
    ...
    real_sums = [s for s in rg_diff_sums if real_number(s)]
    if not real_sums:
      blob.avg_necrotic_value_sum = 0.0
      return
